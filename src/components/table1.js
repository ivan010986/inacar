...
const CustomTable = ({
  ...
  const handleInputChange = async (value, monthIndex, rubroName, inputId) => {
    ...
    setRubrosTotals((prevRubrosTotals) => {
      ...
      updatedTotals[rubroName][monthIndex] =
        (updatedTotals[rubroName][monthIndex] || 0) + difference;
  ...
  const handleUpdatePresupuesto = async () => {
    ...
    const data = Object.keys(inputValues).map((inputId) => {
      ...
      let presupuestomes = isNaN(inputValue?.value) ? 0 : parseInt(inputValue.value);
    ...
    try {
      ...
      for (let i = 0; i < dataChunks.length; i++) {
        const chunk = i === dataChunks.length - 1
          ? dataChunks[i].map((item) => ({
              ...item,
              updatedRubros,
              rubrosTotals,
              monthlyTotals,
      ...
      await Promise.all([
        deleteDataFromDB("selectedPresupuesto", SELECTED_PRESUPUESTO_KEY),
        deleteDataFromDB("rubrosData", RUBROS_DATA_KEY),
      ]);
  
      setSnackbarMessage(`Presupuesto actualizado exitosamente. ${totalUpdated} actualizados, ${totalCreated} creados.`);
      setSnackbarSeverity("success");
    } catch (error) {
