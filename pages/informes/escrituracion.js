...
const CustomerTableVenta = () => {
  ...
  const [rowsByZona, setRowsByZona] = useState(
    zonas.reduce((acc, zona) => ({ ...acc, [zona]: [] }), {})
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      let Promotora = centroCostosData.results.filter(
        (item) => item.uen.nombre === "Promotora"
      );
    ...
    if (selectedCentroCosto && currentZona) {
      ...
      setRowsByZona((prev) => ({
        ...
        [currentZona]: [
          ...prev[currentZona],
          {
            centroDeCosto: centroDeCostoLabel,
            unid: Array(meses.length).fill(0),
            val: Array(meses.length).fill(0),
          },
        ],
      }));
      handleClose();
  ...
  const calculateMonthlyTotals = (zona, type) => {
    return meses.map((_, monthIndex) => {
      return rowsByZona[zona].reduce(
        (total, row) => total + row[type][monthIndex],
        0
      );
    });
  };
  ...
  const exportToExcel = (zona) => {
    const headers = [
      "Centro de costo",
      ...meses.flatMap((mes) => [`Unid ${mes}`, `Val ${mes}`]),
      "Total Unid",
      "Total Val",
    ];
                  ...
                  <Table>
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell>
                          <strong>Total unid</strong>
                        </TableCell>
                        <TableCell>
                          <strong>Total Val</strong>
                        </TableCell>
                        <TableCell>
                    ...
                    <TableBody>
                      {rowsByZona[zona].map((row, rowIndex) => {
                        ...
                        return (
                          <TableRow key={rowIndex}>
                            ...
                            >
                              <Typography>{row.centroDeCosto}</Typography>
                            </TableCell>
                      ...
                      <TableRow>
                        ...
                        {calculateMonthlyTotals(zona, "unid").map(
                          (total, monthIndex) => (
                            <React.Fragment key={monthIndex}>
                              <TableCell>
                                <strong>{total}</strong>
                              ...
                              <TableCell>
                                <strong>
                                  {
                                    calculateMonthlyTotals(zona, "val")[
                                      monthIndex
                                    ]
                                  }
                                </strong>
                        ...
                        <TableCell>
                          <strong>
                            {calculateMonthlyTotals(zona, "unid").reduce(
                              (a, b) => a + b,
                              0
                            )}
                          </strong>
                        ...
                        <TableCell>
                          <strong>
                            {calculateMonthlyTotals(zona, "val").reduce(
                              (a, b) => a + b,
                              0
                            )}
                          </strong>
        ...
        >
          <Autocomplete
            options={
              centroCostos
                .filter(
                  (centroCosto) => centroCosto.regional.nombre === currentZona
                ) // Filtrar por zona
                .map((centroCosto) => ({
            ...
            renderInput={(params) => (
              <TextField {...params} label="Centro de costo" />
            )}
