...
const Promotora = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const savedData = await getDataFromDB("rubrosData", "promotora_rubrosData");
        ...
        if (savedData?.updatedRubros?.length > 0) {
          ...
          await saveDataToDB("promotora_rubrosData", {
        ...
        if (!userId) {
          ...
          const constructoraCentroCostos = centroCostosData.results.filter(item => item.uen.nombre === "Promotora");
