...
const Inmobiliaria = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const savedData = await getDataFromDB("rubrosData", "inmobiliaria_rubrosData");
        ...
        if (savedData?.updatedRubros?.length > 0) {
          ...
          await saveDataToDB("inmobiliaria_rubrosData", {
        ...
        if (!userId) {
          ...
          const constructoraCentroCostos = centroCostosData.results.filter(item => item.uen.nombre === "Inmobiliaria");
...
export default withAuth(Inmobiliaria);